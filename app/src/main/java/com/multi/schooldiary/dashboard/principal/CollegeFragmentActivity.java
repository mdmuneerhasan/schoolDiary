...
public class CollegeFragmentActivity extends AppCompatActivity {
    SavedData savedData;
    Connection connection;
    String id="";
    Date date;
    EditText edtphotoUrl,editText,edtDepartment;
    SimpleDateFormat simpleDateFormat;
    ValueEventListener valueEventListener1;
    private String name,dept,college;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_college_fragment);
...
//                        SetDefaultClass setDefaultClass2=new SetDefaultClass("","","","");
                        SetDefaultClass setDefaultClass=new SetDefaultClass(edtDepartment.getText().toString()+"("+editText.getText().toString()+")","7",null,id);
                        ...
                        connection.getDbSchool().child(id).child("schoolPhoto").setValue(edtphotoUrl.getText().toString());
                        connection.getDbSchool().child(id).child("college").setValue(college);
                        connection.getDbSchool().child(id).child("department").setValue(dept);
            ...
            public void create(View view) {
                dept=edtDepartment.getText().toString();
                college=editText.getText().toString();
                if(college.trim().length()<2){
                    editText.setError("college name too short");
                    return;
                } if(dept.trim().length()<1){
                    edtDepartment.setError("department name too short");
                    return;
                }
                name=dept+" ( "+college+" ) ";
                savedData.showAlert("creating college account...");
